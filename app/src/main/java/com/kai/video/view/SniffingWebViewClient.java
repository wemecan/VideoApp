...
import com.kai.video.sniffing.LogUtil;
...
import com.kai.video.sniffing.node.Node;
...
public class SniffingWebViewClient extends WebViewClient implements SniffingUICallback {
    ...
    public void parserHtml(WebView webView, String url, String html) {
        ...
        if (html.contains(".m3u8")) {
            ...
            if (m3u8.contains(";") || m3u8.contains(",") || m3u8.contains("\"") || m3u8.contains("'") ) {
                ...
                String u = m3u8.split("=")[1];
                if(!mUrls.contains(u)){
                    mUrls.add(u);
                    mVideos.add(new SniffingVideo(u, ".m3u8"));
                    this.onSniffingSuccess(webView, url, mVideos);
                    this.onSniffingFinish(webView, url);
                }
            }else{
                LogUtil.e("SniffingUtil", "onSuccess(containsType)  --> " + url);
                if (!mUrls.contains(m3u8)){
                    mUrls.add(m3u8);
            ...
            if (mp4.contains(";") || mp4.contains(",") || mp4.contains("\"") || mp4.contains("'") ) {
                ...
                LogUtil.e("SniffingUtil", "onSuccess(containsType)  --> " + url);
                String u = mp4.split("=")[1];
                if(!mUrls.contains(u)){
                    mUrls.add(u);
                    mVideos.add(new SniffingVideo(u, ".mp4"));
                    this.onSniffingSuccess(webView, url, mVideos);
                    this.onSniffingFinish(webView, url);
                }
            }else{
                LogUtil.e("SniffingUtil", "onSuccess(containsType)  --> " + url);
                if (!mUrls.contains(mp4)){
                    mUrls.add(mp4);
    ...
    private void parserNode(WebView webView, String url, Node node) {
        ...
        if (!TextUtils.isEmpty(video) || !TextUtils.isEmpty(source) || !TextUtils.isEmpty(ppp_src)) {//找到了video url
            ...
            if ((type = Util.containsType(video)) != null) {
                ...
                new SniffingWebViewClient.ConnectionThread(webView, video, "unknow").start();
            }
            ...
            if (iframes.size() <= 1 && mWebView != null) {
                ...
                mH.postDelayed(mConnTimeout = new SniffingWebViewClient.TimeOutRunnable(webView, url, TYPE_CONN), mConnTimeOut);
    ...
    public void onPageStarted(WebView view, String url, Bitmap favicon) {
        ...
        this.mH.postDelayed(this.mConnTimeout = new SniffingWebViewClient.TimeOutRunnable(view, url, TYPE_CONN), mConnTimeOut);
    ...
    public void onPageFinished(WebView view, String url) {
        ...
        mH.postDelayed(mFinished = new SniffingWebViewClient.FinishedRunnable(view, url), mFinishedTimeOut);
    }

    @Override
    public WebResourceResponse shouldInterceptRequest(WebView view, String url) {
        try {
            LogUtil.e("SniffingUtil1", "shouldInterceptRequest(URL)  --> " + url);
            ...
            if(url.lastIndexOf(".") < url.length() - 5){
                ...
                if (s.equals("application/octet-stream")) {
                    mVideos.add(new SniffingVideo(url, "mp4", (int)content[0], "mp4"));
                }
                if((s.toLowerCase().contains("video") || s.toLowerCase().contains("mpegurl")) && !mUrls.contains(url)){
                    mVideos.add(new SniffingVideo(url,"m3u8",(int) content[0],"m3u8"));
                    mUrls.add(url);
                }
            }else if (mFilter != null) {
                SniffingVideo video = mFilter.onFilter(view, url);
                if (video != null && !mUrls.contains(video.getUrl())){
                    mVideos.add(video);
                    mUrls.add(video.getUrl());
            ...
            //信息不完整，只知道是视频，不知道类型
            if (mVideos.size() > 0){
                String u = mVideos.get(0).getUrl();

                if (u.contains("sf1-ttcdn-tos.pstatp.com") || u.contains("upos-szbyjkm8g1.bilivideo.com") || u.contains("play.m3u8.tv.8old.cn")){
    ...
    public void onSniffingStart(final View webView, final String url) {
        ...
        this.mH.postDelayed(this.mReadTimeout = new SniffingWebViewClient.TimeOutRunnable((WebView) webView, url, TYPE_READ), mReadTimeOut);
    ...
    private class FinishedRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (mVideos.isEmpty()) {
                ...
                mH.postDelayed(new SniffingWebViewClient.ParserHtmlRunnable(view, "alert"), 5000);
                mH.postDelayed(mJSRunnable = new SniffingWebViewClient.ParserHtmlRunnable(view, "confirm"), 6000);
            } else {
    ...
    private class TimeOutRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (type == TYPE_CONN) {
                ...
                mConnTimeout = null;
                mH.postDelayed(new SniffingWebViewClient.ParserHtmlRunnable(view, "alert"), 5000);
                mH.postDelayed(mJSRunnable = new SniffingWebViewClient.ParserHtmlRunnable(view, "confirm"), 8000);
        ...
        public void run() {
            try {
                ...
                if (content[1] == null) {
                    ...
                    mH.postDelayed(mConnTimeout = new SniffingWebViewClient.TimeOutRunnable(view, url, TYPE_CONN), mConnTimeOut);
